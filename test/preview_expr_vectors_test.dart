...
void main() {
  ...
  group('expression vectors', () {
    test('the fixture carries the full contract', () {
      expect(vectors.length, greaterThanOrEqualTo(minimumVectorCount),
          reason: 'expected at least $minimumVectorCount vectors, found ${vectors.length}');
    });

    for (int index = 0; index < vectors.length; index++) {
      final Map<String, Object?> vector = vectors[index]! as Map<String, Object?>;
      final String source = vector['expr']! as String;
      final bool expectError = vector['error'] == true;
      final Map<String, Object?> rawVars =
          (vector['vars'] as Map<String, Object?>?) ?? const <String, Object?>{};
      final Map<String, Object?> vars = rawVars.map(
          (String key, Object? value) => MapEntry<String, Object?>(key, toRuntimeValue(value)));
      ...
      test('[$index] $source', () {
        ...
        if (expectError) {
          ...
          // both "error": true.
          expect(() => evalPreviewExpr(parsePreviewExpr(source), scope),
...
void expectMatches(String source, Object? expected, Object actual) {
  if (expected is bool) {
    expect(actual, isA<bool>(), reason: "vector '$source': expected boolean result");
    expect(actual, expected, reason: "vector '$source'");
    return;
  }
  if (expected is num) {
    expect(actual, isA<double>(), reason: "vector '$source': expected numeric result");
    expect(actual as double, closeTo(expected.toDouble(), delta), reason: "vector '$source'");
    return;
  }
  ...
  if (text.startsWith('#')) {
    ...
    expect(actual, isA<double>(), reason: "vector '$source': expected colour result");
    final double expectColor = (parsePreviewExpr(text) as PNum).value;
    expect(actual as double, closeTo(expectColor, delta), reason: "vector '$source'");
    return;
  }
  expect(actual, isA<String>(), reason: "vector '$source': expected string result");
