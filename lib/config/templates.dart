...
///
/// Every template is authored against the Java parser's real behaviour, not the
/// shipped JSON schema: sound actions always carry a source plus volume/pitch 1,
/// command actions always carry a source, item keys are lowercase registry keys,
...
const List<HuiTemplate> huiTemplates = <HuiTemplate>[
  ...
  HuiTemplate(
    ...
    description:
        ...
        'pick an icon and how the true/false actions fire. The close button '
        'runs /holoui close, so players need holoui.command and '
        'holoui.command.close (both default to op).',
...
HuiComponent _text(String id, double x, double y, String text) => HuiComponent(
      id,
      Vec3(x, y, 0),
      HuiDecorationData(HuiTextIcon(text)),
    );
...
HuiMenu buildSettingsTemplate() => _menu(<HuiComponent>[
      ...
      _text(
        ...
        '&7Conditions are read once, when the menu opens.\n'
            '&8Requires PlaceholderAPI and EssentialsX.',
      ),
      _commandButton(
        'close',
        0,
