...
final class GlossAnimationPlayer {
  ...
  int frameIndex(GlossAnimationDoc doc, String id) {
    if (doc.frames.isEmpty) return 0;
    if (_playing) return glossAnimationFrameIndexAt(doc, id, _clock());
    return _heldIndex.clamp(0, doc.frames.length - 1);
  }

  String frameText(GlossAnimationDoc doc, String id) =>
      doc.frames.isEmpty ? '' : doc.frames[frameIndex(doc, id)];

  /// Freezes on whatever frame the clock shows at this instant.
  void pause(GlossAnimationDoc doc, String id) {
    if (!_playing) return;
    _heldIndex = frameIndex(doc, id);
    _playing = false;
  }
