...
class BoardView extends StatefulWidget {
  const BoardView({required this.store, super.key});
...
class _BoardViewState extends State<BoardView> {
  ...
  @override
  void didUpdateComponent(BoardView oldComponent) {
  ...
  Widget _body() {
    ...
    return dom.div(classes: 'hui-board', <Widget>[
      _toolbar(board, graph),
      if (decoded.warning != null)
        _notice(decoded.warning!, 'is-warning')
      else if (graph.scopeMissing)
        _notice(
          'This board\'s saved folder no longer exists. All menus are shown.',
          'is-warning',
        ),
      _summary(graph),
      dom.div(classes: 'hui-board-body', <Widget>[
        if (board.view == WorkspaceBoardView.canvas)
          _canvas(board, graph)
        else
          _list(graph),
      ]),
  ...
  Widget _toolbar(WorkspaceBoardData board, WorkspaceFlowGraph graph) =>
      dom.div(classes: 'hui-board-toolbar', <Widget>[
        ...
        dom.div(classes: 'hui-board-controls', <Widget>[
          dom.div(classes: 'hui-board-scope', <Widget>[
            ...
            ArcaneSelect(
              value: board.scopeFolderId ?? _allFoldersValue,
              size: ComponentSize.sm,
              onChange: (String value) => _setScope(board, value),
          ...
          Button.outline(
            ...
            label: 'Arrange',
            onPressed: () => _arrange(board, graph),
          ),
          ArcaneToggleGroup(
            value: board.view.name,
            variant: ToggleGroupVariant.outline,
            size: ToggleGroupSize.sm,
            onChanged: (String? value) => _setView(board, value),
            items: <ToggleGroupItem>[
              ToggleGroupItem(
                value: WorkspaceBoardView.canvas.name,
                child: ArcaneIcon.share2(size: IconSize.sm),
              ),
              ToggleGroupItem(
                value: WorkspaceBoardView.list.name,
  ...
  Widget _canvas(WorkspaceBoardData board, WorkspaceFlowGraph graph) {
    if (graph.documents.isEmpty) return _emptyGraph();
    final Map<String, WorkspaceBoardPoint> automatic =
    ...
    final Map<String, WorkspaceBoardPoint> positions =
        <String, WorkspaceBoardPoint>{
          for (final WorkspaceDoc doc in graph.documents)
            doc.id: board.positions[doc.id] ?? automatic[doc.id]!,
        };
    double maxX = 0;
    double maxY = 0;
    for (final WorkspaceBoardPoint point in positions.values) {
      maxX = math.max(maxX, point.x);
      maxY = math.max(maxY, point.y);
    }
    final Map<WorkspaceFlowTargetKind, WorkspaceBoardPoint> sinks =
        _sinkPositions(graph, maxX + 340);
    for (final WorkspaceBoardPoint point in sinks.values) {
      maxY = math.max(maxY, point.y);
    }
    ...
    return dom.div(classes: 'hui-board-scroll', <Widget>[
      dom.div(
        ...
        <Widget>[
          ...
          for (final MapEntry<WorkspaceFlowTargetKind, WorkspaceBoardPoint> sink
              in sinks.entries)
  ...
  Widget _edge(
    WorkspaceFlowEdge edge,
    WorkspaceBoardPoint source,
    WorkspaceBoardPoint target,
  ) {
  ...
  Widget _sink(WorkspaceFlowTargetKind kind, WorkspaceBoardPoint point) =>
      dom.div(
  ...
  Map<WorkspaceFlowTargetKind, WorkspaceBoardPoint> _sinkPositions(
    WorkspaceFlowGraph graph,
    double x,
  ) {
    final Map<WorkspaceFlowTargetKind, WorkspaceBoardPoint> out =
        <WorkspaceFlowTargetKind, WorkspaceBoardPoint>{};
    int row = 0;
    for (final WorkspaceFlowTargetKind kind in const <WorkspaceFlowTargetKind>[
      ...
      out[kind] = WorkspaceBoardPoint(x, 72 + row * 94);
      row++;
    }
    return out;
  }
  ...
  Widget _emptyGraph() => const dom.div(classes: 'hui-board-empty', <Widget>[
    Text('No menu documents are inside this board scope.'),
  ]);
  ...
  void _setView(WorkspaceBoardData board, String? raw) {
    for (final WorkspaceBoardView view in WorkspaceBoardView.values) {
      if (view.name == raw) {
        component.store.updateBoard(board.copyWith(view: view));
        return;
      }
    }
  }

  void _setScope(WorkspaceBoardData board, String raw) {
    component.store.updateBoard(
      board.copyWith(
        scopeFolderId: raw == _allFoldersValue ? null : raw,
        clearScope: raw == _allFoldersValue,
        positions: const <String, WorkspaceBoardPoint>{},
      ),
    );
  }

  void _arrange(WorkspaceBoardData board, WorkspaceFlowGraph graph) {
    component.store.updateBoard(
      board.copyWith(positions: arrangeWorkspaceFlowGraph(graph)),
    );
  }
}
