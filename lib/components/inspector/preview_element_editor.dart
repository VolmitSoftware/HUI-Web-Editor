...
class PreviewElementEditor extends StatelessWidget {
  ...
  Set<String> get _scope {
    ...
    final String name =
        (repeat.varName == null || repeat.varName!.isEmpty) ? 'i' : repeat.varName!;
    return <String>{name};
  }
  ...
  Widget build(BuildContext context) => dom.div(
        classes: 'hui-inspector-body is-preview-element',
        <Widget>[
          _header(),
          _placement(),
          _repeatSection(),
  ...
  Widget _header() {
    final int errors =
        _elementIssues.where((HuiIssue i) => i.severity == HuiSeverity.error).length;
    final int warnings =
        _elementIssues.where((HuiIssue i) => i.severity == HuiSeverity.warning).length;
    return dom.div(
        ...
        dom.div(
          ...
          <Widget>[
            ...
            dom.h2(
              ...
              <Widget>[Text(_typeLabel(element.type))],
            ),
          ],
        ),
  ...
  Widget _placement() => InspectorSection(
        title: 'Placement',
        children: <Widget>[
          _field('X', element.x, (Object? v) => _mutate('x', (HuiPreviewElement e) => e.x = v),
              field: 'x', placeholder: '0'),
          _field('Y', element.y, (Object? v) => _mutate('y', (HuiPreviewElement e) => e.y = v),
              field: 'y', placeholder: '0'),
          _field('Z', element.z, (Object? v) => _mutate('z', (HuiPreviewElement e) => e.z = v),
              field: 'z',
              placeholder: _formatDefault(_defaultZ()),
              help: 'Higher z draws in front. Left blank, this type defaults '
                  'to ${_formatDefault(_defaultZ())}.'),
          _field(
            'Visible',
            element.visible,
            (Object? v) => _mutate('visible', (HuiPreviewElement e) => e.visible = v),
            field: 'visible',
            kind: PreviewExprKind.boolean,
            placeholder: 'true',
            help: 'Evaluated once when the preview opens - hiding an element '
                'at build time keeps it hidden for the life of that preview.',
          ),
        ],
      );

  static String _formatDefault(double value) =>
  ...
  Widget _repeatSection() {
    ...
    return InspectorSection(
      title: 'Repeat',
      description: 'Emits this element several times with an index bound to '
      ...
      children: <Widget>[
        HuiSwitchRow(
          ...
          onChanged: (bool value) => _mutate(
            ...
            (HuiPreviewElement e) =>
                e.repeat = value ? HuiPreviewRepeat(count: 2, varName: 'i') : null,
          ),
        ),
        if (repeat != null) ...<Widget>[
          PreviewExprField(
            ...
            required: true,
            placeholder: '2',
            help: 'Evaluated once, with no variable scope of its own - it '
          ...
          HuiField(
            label: 'Loop variable',
            help: 'Defaults to "i". Must be a plain identifier and cannot '
  ...
  Widget _panelSection() => InspectorSection(
        title: 'Panel',
        children: <Widget>[
          _field('Width', element.width,
              (Object? v) => _mutate('width', (HuiPreviewElement e) => e.width = v),
              field: 'width', required: true, placeholder: '40'),
          _field('Height', element.height,
              (Object? v) => _mutate('height', (HuiPreviewElement e) => e.height = v),
              field: 'height', required: true, placeholder: '20'),
          _field('Color', element.color,
  ...
  Widget _cellSection() => InspectorSection(
        title: 'Cell',
        children: <Widget>[
          _field('Size', element.size,
              (Object? v) => _mutate('size', (HuiPreviewElement e) => e.size = v),
              field: 'size', required: true, placeholder: '8'),
          _field(
            'Color',
            element.color,
            (Object? v) => _mutate('color', (HuiPreviewElement e) => e.color = v),
            ...
            placeholder: '#FF15151B',
            swatch: element.color,
            help: 'The one field that is re-evaluated live, every 4 ticks - '
  ...
  Widget _slotSection() => InspectorSection(
        ...
        children: <Widget>[
          _field('Size', element.size,
              (Object? v) => _mutate('size', (HuiPreviewElement e) => e.size = v),
              field: 'size', required: true, placeholder: '18'),
          _field('Index', element.index,
              (Object? v) => _mutate('index', (HuiPreviewElement e) => e.index = v),
              field: 'index',
              required: true,
              placeholder: '0',
              help: 'Which inventory slot to draw. Nothing clamps this - '
                  'guard it against inventory.size yourself.'),
          _field(
            'Well color',
            element.wellColor,
            (Object? v) => _mutate('wellColor', (HuiPreviewElement e) => e.wellColor = v),
            field: 'wellColor',
  ...
  Widget _labelSection() => InspectorSection(
        ...
        children: <Widget>[
          _field(
            ...
            (Object? v) => _mutate(
              'text',
              (HuiPreviewElement e) => e.text = v as String?,
            ),
            field: 'text',
            required: true,
            kind: PreviewExprKind.string,
            placeholder: "'Label text'",
            help: 'The other field that is re-evaluated live, every 4 ticks. '
                'A string literal needs its own quotes: \'like this\'.',
          ),
          _field(
            'Background',
            element.background,
            (Object? v) => _mutate('background', (HuiPreviewElement e) => e.background = v),
            field: 'background',
