...
///
/// Issues come from `validateHuiMenu`, which encodes runtime behavior directly
/// rather than treating the hand-maintained JSON schema as executable truth.
library;
...
class ValidationPanel extends StatelessWidget {
  ...
  List<Widget> _group(HuiSeverity severity, List<HuiIssue> all) {
    final List<HuiIssue> group = all
        .where((HuiIssue issue) => issue.severity == severity)
        .toList();
      ...
      dom.div(
        ...
        <Widget>[
          ...
          dom.ul(classes: 'hui-validation-list', <Widget>[
            for (final HuiIssue issue in group) _row(issue),
          ]),
        ],
      ),
    ];
  ...
  Widget _row(HuiIssue issue) {
    ...
    return dom.li(classes: 'hui-validation-row', <Widget>[
